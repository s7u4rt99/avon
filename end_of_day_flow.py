...
def generate_all_summary(email: str):
    ...
    if user is None:
        ...
        events_str = generate_event_summary(
...
def start_night_flow(email: str):
    ...
    unfinished_tasks = [
        ...
        if (task.get("completed", False) == False)
        and (task.get("type", "task") == "task")
    ]
    habit_goals = [
        task
        for task in task_list
        if (task.get("completed", False) == False)
        and (task.get("type", "task") == "habit")
    ]
    ...
    for task in schedule_obj["tasks"]:
        task_line = "\n{0}. {1}\n   - Time Slot: {2}".format(
            task["id"], task["task_name"], task["time_slot"]
        )
        formatted_tasks += "\n----------------" + task_line
