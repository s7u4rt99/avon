...
def message(user_id: int, message: str):
    return (
        ...
        .insert({"user_id": user_id, "message": message})
        .execute()
    )
    ...
    if todo.state != "adjusting":
        if todo.type == "tasks":
            if todo.deadline == None:
                ...
                write_chat_message(
                    get_user(todo["user_id"])["email"], message, f"/reply/{todo.id}"
                )
                return
        ...
        midnight = datetime(time_now.year, time_now.month, time_now.day, 23, 59, 59)
        ...
        edit_task(
            task_id=todo.id,
            adjusted_start=resParsed["start_time"],
            adjusted_end=resParsed["end_time"],
        )
...
def add_new_task_flow(message: str):
    ...
    if resParsed["type"] == "Task":
        ...
        estimated_duration = (
            None
            if resParsed["estimated_duration"] == "none"
            else resParsed["estimated_duration"]
        )
        todo = (
            supabase.table("Tasks")
            .insert(
        ...
        type = "events"
        event_start = (
            None if resParsed["start_time"] == "none" else resParsed["start_time"]
        )
